...
class PointInfo(object):
    ...
    def update_val2cnt(self, new_val2cnt): #new_val2cnt is dict(Counter)
    ...
    def get_mean(self):
        ...
        if total_cnt == 0:
            return None
        return total_val / total_cnt # a float scalar
...
class PointInfoManager(object):
    ...
    def get_real_T(self, excludes_none=False, gets_idx=False):
        ...
        if excludes_none:
            return (real_T[observed_idx], observed_idx) if gets_idx else real_T[observed_idx]
        else:
            # observed_idx still contains indices of None points even though real_T contains no None
            return (real_T, observed_idx) if gets_idx else real_T
...
#            print("point normal\n", [point_info.get_total_zero_mean_unit_var_normalized_val(self.T_mean, self.T_std) for point_info in self.point_info_list])
            ...
            # self.set_normalized_params()
            return np.array(
                [point_info.get_total_zero_one_normalized_val(self.T_lower, self.T_upper) 
                    for point_info in self.point_info_list])
        else:
    ...
    def update(self, coordinate, val2cnt): #val2cnt is dict(Counter)
